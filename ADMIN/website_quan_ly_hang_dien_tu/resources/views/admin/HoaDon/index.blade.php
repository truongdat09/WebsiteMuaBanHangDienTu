...
@section('content')
    <div class="card mx-3">
        <div class="card-body">
            ...
            <form method="get" action="{{ route('admin.hoadon.listByDate') }}">
                @csrf
                <label for="start_date">Từ ngày:</label>
                <input type="date" id="start_date" name="start_date" required>

                <label for="end_date">Tới ngày:</label>
                    ...
                    <tbody>
                        @foreach ($hoadons as $hoadon)
                            <tr>
                                ...
                                <td>
                                    <select name="TrangThai[]" id="TrangThai_{{ $hoadon->MAHD }}" onchange="confirmUpdate('{{ $hoadon->MAHD }}', '{{ $hoadon->MaTT }}')">
                                        @foreach ($trangthaiList as $trangthai)
                                            <option value="{{ $trangthai->MaTT }}" {{ $hoadon->MaTT == $trangthai->MaTT ? 'selected' : '' }}>
                                                {{ $trangthai->TrangThai }}
                                            </option>
                                        @endforeach
                                    </select>
    ...
    <script>
        function confirmUpdate(MAHD, currentTrangThai) {
            var selectId = "TrangThai_" + MAHD;
            var selectElement = document.getElementById(selectId);

            if (selectElement && confirm("Bạn có muốn thay đổi trạng thái không?")) {
                var newTrangThai = selectElement.value;
